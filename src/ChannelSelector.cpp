...
ChannelSelector::ChannelSelector(
    ...
    std::map<std::string, Channel *> *masterChannelList) {
  connectionsWaiting = waiting;
...
void ChannelSelector::setStatus(int s) { status = s; }
    ...
    while (status) {
      ...
      if (bytes > 0) {
        ...
        if (newConnection->handshake(buffer, &attributes) == 0) {
          ...
          std::cout<<"[Response] " << std::endl << attributes.response <<
          std::endl;
          */
          // std::cout<<"[Channel] " << attributes.channel << std::endl;
          newConnection->send(attributes.response.c_str(),
                              attributes.response.length());
          if (channels->find(attributes.channel) != channels->end()) {
            // std::cout<< "Connection sent to: " <<
            // attributes.channel<<std::endl;
