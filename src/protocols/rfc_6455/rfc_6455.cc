...
int RFC6455::handshake(const std::string input, WSAttributes * response) {
  ...
  response->version = input.substr(verStart + versionLookupStart.length(),
                                   verEnd -
                                   (verStart + versionLookupStart.length()));
  ...
  std::string inputKey = input.substr(keyStart + keyLookupStart.length(),
                                       keyEnd - (keyStart +
                                          keyLookupStart.length())) + GUID;
...
unsigned long RFC6455::packetRealLength(const std::string input) {
  ...
  while (!tmp.empty() && !foundEndPacket) {
    if (packetFragmented(tmp)) {
      // Packet is a partial let's check to make sure all the
      //    packets are here before we allow decoding.
...
void RFC6455::packetLength(const std::string input, WSPacketLength * pcktLen) {
  ...
  */
  ...
  if (pcktLen->length == 126) {
    ...
    pcktLen->packetLen = pcktLen->payloadOffset +
                             (hasMask(input)? 4: 0) + pcktLen->length;
  } else
  if (pcktLen->length == 127) {
    // 127 - 8 bytes
    // NOT TESTED || Need a test packet.
    // std::cout<<"Ver: 127 Size"<<std::endl;
    pcktLen->length = inputBytes[2] << 24 |
                      inputBytes[3] << 16 |
                      inputBytes[4] << 8  |
                      inputBytes[5];
  ...
  do {
    ...
    if (hasMask(unknownPacket)) {
      ...
      while (byteCounter < pcktLen.length) {
        ...
        // std::cout << " " <<
        //    (unsigned long)(inputBytes[byteCounter+offset] ^
    ...
    // std::cout << "Length of unknownPacket: "
    //           << unknownPacket.size() << std::endl;
