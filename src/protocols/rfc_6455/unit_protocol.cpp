#include <iostream>
#include <string>
#include <cassert>
#include "../WSProtocol.h"
#include "RFC_6455.h"

using namespace std;

int main ( ) { 
	WSProtocol * protocolTest = new RFC_6455();
	WSAttributes attributes;
	string exampleInput = "GET /chat HTTP/1.1\r\nHost: server.example.com\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nOrigin: http://example.com\r\nSec-WebSocket-Protocol: chat, superchat\r\nSec-WebSocket-Version: 13";
	string exampleResponse = "HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\n\r\n";

	int verCheck = protocolTest->handshake( exampleInput , &attributes );
	
	assert ( verCheck == 0 );
	cout<<"Version Check Passed..."<<endl;
	assert ( attributes.channel.compare("chat") == 0 );
	cout<<"Channel Check Passed..."<<endl;
	assert ( exampleResponse.compare(attributes.response) == 0 );
	cout<<"Handshake Passed..."<<endl;
	/*
	
   	A single-frame unmasked text message

         0x81 0x05 0x48 0x65 0x6c 0x6c 0x6f (contains "Hello")v

        A single-frame masked text message

         0x81 0x85 0x37 0xfa 0x21 0x3d 0x7f 0x9f 0x4d 0x51 0x58
         (contains "Hello")

	
	*/
	string exampleWSMessage_small = "\x81\x05\x48\x65\x6c\x6c\x6f";
	string exampleWSMessage_small_payload = "Hello";
	string exampleWSMessage_split_1 = "\x01\x03\x48\x65\x6c";
	string exampleWSMessage_split_2 = "\x80\x02\x6c\x6f";
	string exampleWSMessage_split_payload = "Hello";
	string exampleWSMessage_long = std::string("", 1303);
	string exampleWSMessage_long_payload = "data:image/webp;base64,";
	string decodedPayload;
	string encodedPayload;

	std::cout << "Case 1...testing....decoding(SMALL)...";
	decodedPayload = protocolTest->decode ( exampleWSMessage_small );
	assert ( decodedPayload.compare(exampleWSMessage_small_payload) == 0 );
	std::cout << "ok" << std::endl;

	std::cout << "Case 2...testing....encoding(SMALL)...";
	//Re-encode and then decode again.
	encodedPayload = protocolTest->encode ( decodedPayload );
	decodedPayload = protocolTest->decode ( encodedPayload );
	assert ( decodedPayload.compare(exampleWSMessage_small_payload) == 0 );
	std::cout << "ok" << std::endl;	

	std::cout << "Case 3...testing....decoding(LARGE)...";
	decodedPayload = protocolTest->decode ( exampleWSMessage_long );
	assert ( decodedPayload.compare(exampleWSMessage_long_payload) == 0 );
	std::cout<< "ok" << std::endl;

	std::cout << "Case 4...testing....decoding an encoded message...";
	encodedPayload = protocolTest->encode ( exampleWSMessage_long_payload );
	decodedPayload = protocolTest->decode ( encodedPayload );
	assert ( decodedPayload.compare(exampleWSMessage_long_payload) == 0 );
	std::cout << "ok" << std::endl;

	/*
        A fragmented unmasked text message

         0x01 0x03 0x48 0x65 0x6c (contains "Hel")

         0x80 0x02 0x6c 0x6f (contains "lo")
    */

	std::cout << "Case 5...testing....framented packets" << std::endl;
	std::cout << "--testing (1/2) fragment of packet...";
	assert ( protocolTest->packetComplete ( exampleWSMessage_split_1 ) == 0 );
	std::cout << "ok" << std::endl;

	std::cout << "Case 6...testing....fragmented packets" << std::endl;
	std::cout << "--testing (2/2) fragment of packet...";
	assert ( protocolTest->packetComplete ( exampleWSMessage_split_1+exampleWSMessage_split_2 ) == 1 );
	std::cout << "ok" << std::endl;

	std::cout << "Case 7...testing....fragmented packets" << std::endl;
	std::cout << "--testing decoding of fragmented packet...";
	assert ( exampleWSMessage_split_payload.compare ( protocolTest->decode ( exampleWSMessage_split_1+exampleWSMessage_split_2 ) ) == 0 );
	std::cout << "ok" <<std::endl;
	
	return 0;
}
