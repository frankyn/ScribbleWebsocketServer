...
function onMessage ( descriptor, msg )
	local t = json.decode ( msg )
	if tostring(t.id) == "1" then
		data = json.decode ( getid ( t.user ) ) 
		if ( t.pass == data.password ) then
			data.user = t.user
			users[descriptor] = data
			td = {}
			td.id = 1
			td.status = 1
			msg = json.encode ( td )
			sendTo ( descriptor, msg )
		else 
			td = {}
			td.id = 1
			td.status = 2
	...
	elseif tostring(t.id) == "2" then
		if getid ( t.user )=="" then
			td = {}
			td.password = t.pass
			td.email = t.email
			storeid ( t.user, json.encode ( td ) )
			td = {}
			td.id = 2
			td.status = 1
			sendTo ( descriptor, json.encode ( td ) )
		else
	...
	elseif tostring(t.id) == "3" then
		if users[descriptor] then
			t.pID = descriptor 
			broadcast ( descriptor, json.encode ( t )  )
		end
	elseif tostring(t.id) == "4" then
		if users[descriptor] then
			t.user = users[descriptor].user
			broadcast ( descriptor, json.encode ( t ) )
		end
	end
	return ""
