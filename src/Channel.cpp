...
Channel::Channel(int channel_id, std::string db_key, std::string channel_name,
                 std::string script) {
  ...
  /*Channel LUA API*/
  logicModule.addProc(&Channel::luaBroadcast, (void *)this, "broadcast");
  logicModule.addProc(&Channel::luaDisconnectUser, (void *)this,
                      "disconnectUser");
...
Channel::~Channel() { connections.clear(); }

int Channel::usersConnected() { return connections.size(); }

void Channel::setStatus(int status) { this->status = status; }

std::string Channel::currentScript() { return scriptFile; }
    ...
    for (int ce = 0; ce < eventsOccuring; ++ce) {
      ...
      if ((buffer_len = conn->recv(buffer, sizeof(buffer) - 1)) == 0) {
        ...
        if (msgLength != 0 && conn->packetComplete(conn->getBuffer())) {
          // We have a complete packet waiting for us so we need clear it from
          // the buffer.
...
MySQL *Channel::getDB() { return &appDatabase; }

std::string Channel::getName() { return this->name; }

int Channel::getID() { return this->channelID; }

std::string Channel::getDBKey() { return this->channelDBKey; }
...
int Channel::luaStore(lua_State *state) {
  ...
  if (db->hasNext()) {
    // It has values
    // Update Value
    if (!db->exec("UPDATE appData SET value = '" + value + "' WHERE key = '" +
                  key + dbKey + "'")) {
      // error
    ...
    if (!db->exec("INSERT INTO appData  ( key, value ) VALUES ( '" + key +
                  dbKey + "','" + value + "' )")) {
      // error
      lua_pushnumber(state, 0);
    }
