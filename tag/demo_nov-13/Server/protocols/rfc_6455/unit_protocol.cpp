...
int main ( ) { 
	...
	*/
	...
	string exampleWSMessage_2 = "\x81\x05\x48\x65\x6c\x6c\x6f\x81\x05\x48\x65\x6c\x6c\x6f";
	string examplePayload = "Hello";
	
	string decodedMessage = protocolTest->decode ( exampleWSMessage );
	unsigned messageLength = protocolTest->packetLength ( exampleWSMessage );
	
	assert ( exampleWSMessage.size() == messageLength );
